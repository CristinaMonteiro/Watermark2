...
import subprocess
import json
import string #para o gerar do token
import random #para o gerar do token
...
def is_strong_password(password):
    # No mínimo: 8 caracteres, 1 maiúscula, 1 minúscula, 1 número, 1 símbolo
    return bool(re.match(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$', password))

#para criar o token de 7 caracteres associado ao user
...
def register():
    try:
        ...
        #usa a funcao de gerar tokens e cria um associado ao user
        token_user = generate_token(7)
        #garante que o token criado nao existe na bd
        while collectionUsers.find_one({'token_user': token_user}):
            token_user = generate_token(7)
...
def login():
    ...
    if user and bcrypt.check_password_hash(user['password'], password):
        ...
        token_user = user.get('token_user', None) #vai buscar o token_user do utilizador

        return jsonify({
            'access_token': access_token,
            'token_user': token_user, #vai buscar o token_user do utilizador para poder assinar com ele
...
def infer_torchserve():
    try:
        ...
        if len(secret) > 7:
            return jsonify({"error": "Segredo demasiado longo. Máximo 7 caracteres."}), 400
...
def verify_torchserve():
    try:
        ...
        # Envia imagem para o TorchServe (sem segredo)
        ts_url = "http://localhost:8080/predictions/autoencoder"
        headers = {"Content-Type": "application/json"}
        ...
        response_data = response.json()
        print("Resposta TorchServe (verify):", response_data)

        has_secret = response_data.get("has_secret", False)
        secret = response_data.get("secret", None)

        # 🔍 Verifica se esse secret corresponde a algum utilizador
        owner_info = None
        if has_secret and secret:
            user_found = collectionUsers.find_one({"token_user": secret})
            if user_found:
                owner_info = {
                    "nif": user_found.get("nif"),
        ...
        return jsonify({
            ...
            "secret": secret,
            "owner": owner_info  # devolve o dono se encontrado
        }), 200
